...
export default function Dashboard({ onBack, config, onConfigUpdate }: Props) {
    const { currentProject, clearCurrentProject } = useCurrentProject();
    const [choice, setChoice] = useState<ActionChoice | null>(null);
    const [deleteRemoteRepoOnDelete, setDeleteRemoteRepoOnDelete] = useState<
        boolean | null
    >(null);
    ...
    const [selectedEnvironmentId, setSelectedEnvironmentId] = useState<
        string | null
    >(null);
    const [confirmDestroyEnvironmentId, setConfirmDestroyEnvironmentId] =
        useState<string | null>(null);
    const [confirmDriftEnvironmentId, setConfirmDriftEnvironmentId] = useState<
        string | null
    >(null);
    const [pendingApplyEnvironmentId, setPendingApplyEnvironmentId] = useState<
        string | null
    >(null);
    const [editReleaseEnvironmentId, setEditReleaseEnvironmentId] = useState<
        string | null
    >(null);
    ...
    >([]);
    const [selectedEnvironmentProvider, setSelectedEnvironmentProvider] =
        useState<string | null>(null);
    const [suggestedReleaseTag, setSuggestedReleaseTag] = useState<string>("");
    const [releaseBuildMonitor, setReleaseBuildMonitor] =
        useState<ReleaseBuildMonitorState | null>(null);
    const [deploymentNotice, setDeploymentNotice] = useState<string | null>(
        null
    );
    const [deploymentError, setDeploymentError] = useState<string | null>(null);
    const [deploymentLogs, setDeploymentLogs] = useState<string[]>([]);
    const [deploymentPlannedChanges, setDeploymentPlannedChanges] = useState<
        PlannedChange[]
    >([]);
    const [deploymentSteps, setDeploymentSteps] = useState<DeploymentStep[]>(
        []
    );
    const [deploymentStartedAt, setDeploymentStartedAt] = useState<
        number | null
    >(null);
    ...
    useEffect(() => {
        ...
        let active = true;
        const localUrl = resolveOpenUrl(currentProject);
        const intervalMs = 5000;

        const probeLocalRuntime = async () => {
            if (isDockerized) {
                const dockerPs = await callShell(
                    "docker",
                ...
                if (active) setIsLocalRunning(running);
                return;
            }
            ...
            try {
                ...
                if (active) {
                    setIsLocalRunning(response.ok || response.status > 0);
                }
            } catch {
                if (active) setIsLocalRunning(false);
            } finally {
    ...
    }, [currentProject, isDockerized]);

    const isOnAppDashboardHome =
        !choice &&
        !selectedEnvironmentId &&
        !confirmDestroyEnvironmentId &&
        !confirmDriftEnvironmentId &&
        !pendingApplyEnvironmentId &&
        !editReleaseEnvironmentId &&
        !releaseBuildMonitor;
    ...
    const deployWorkspaceReadiness = useMemo(
        () =>
            currentProject
                ? evaluateDeployWorkspaceReadiness(currentProject.path)
                : null,
        [currentProject?.path, deploymentRefreshKey]
    );

    useEffect(() => {
        if (!currentProject || !selectedEnvironmentId) return;
        ...
        try {
            ...
            setDeploymentError(
                getErrorMessage(
                    error,
                    "Failed to generate deploy workspace for this environment."
                )
            );
        }
        ...
        try {
            ...
            if (!deleteGuard.allowed) {
                const environmentIds = Array.from(
                    new Set(
                        deleteGuard.blocks.map((block) => block.environmentId)
                    )
                ...
                for (const environmentId of environmentIds) {
                    ...
                    if (environmentId === "prod") {
                        destroyArgs.push(
                            "--confirm-prod",
                            "destroy prod permanently"
                        );
                    }
                    const destroyLogs: string[] = [];
                    const destroyResult = await maybeRunDeploymentsCommand(
                        destroyArgs,
                        {
                            ...
                            writeLine: (line) => {
                                ...
                                if (normalized.length > 0)
                                    destroyLogs.push(normalized);
                            },
                        }
                    );
                    if (
                        !destroyResult.handled ||
            ...
            const stopDockerIfPossible = async () => {
                if (!isDockerized || !hasDockerComposeFile(currentProject.path))
                    return;
                await callShell(
                    "docker",
            ...
            if (deleteRemoteRepoOnDelete) {
                const remoteDeleteResult =
                    await maybeDeleteGithubRepoForProject(currentProject.path);
                ...
                if (
                    remoteDeleteResult.message.startsWith(
                        "Failed to delete remote GitHub repo"
                    )
                ) {
    ...
    const isEnvironmentDownFromLatestRun = (environmentId: string): boolean => {
        const latestRun = (projectStateConfig?.deploymentRunHistory ?? [])
            .filter((run) => run.environmentId === environmentId)
            .sort((a, b) => b.finishedAt.localeCompare(a.finishedAt))[0];
        return (
            latestRun?.action === "destroy" && latestRun?.status === "success"
        );
    };

    const getEnvironmentStatus = (environmentId: string): string => {
        const status =
            projectStateConfig?.deploymentState?.environments?.[environmentId]
                ?.lastStatus;
        if (status === "unknown" && isEnvironmentDownFromLatestRun(environmentId)) {
            return "down";
        }
        if (status) return status;
        if (isEnvironmentDownFromLatestRun(environmentId)) {
            return "down";
        }
    ...
    const getEnvironmentStatusLabel = (environmentId: string): string => {
        ...
        const releaseTag = releaseSelection?.selectedReleaseTag?.trim();
        if (!releaseTag) {
            return status;
        }

        const presetName = releaseSelection?.selectedDeployPresetId?.trim();
        return presetName ? `${releaseTag} (${presetName})` : releaseTag;
    };

    const getEnvironmentUrl = (environmentId: string): string => {
        const appBaseUrlRecord =
            projectStateConfig?.environmentOutputs?.[environmentId]
                ?.APP_BASE_URL;
        if (
            typeof appBaseUrlRecord?.value === "string" &&
            appBaseUrlRecord.value.trim().length > 0
        ) {
            return appBaseUrlRecord.value.trim();
        }
        return "-";
    };
    ...
    const runDeploymentAction = async (
        ...
        exitCode: number;
        logs: string[];
        planChanges?: PlannedChange[];
    }> => {
        const now = Date.now();
        if (
            deploymentInProgress ||
            (!options?.bypassCooldown &&
                now < deploymentActionCooldownUntilRef.current)
        ) {
    ...
    const runApplyForEnvironment = async (
        ...
        options: {
            ...
            getStepStatus: (
                id: DeploymentStepId
            ) => DeploymentStepStatus | undefined;
        }
    ): Promise<void> => {
        ...
        if (result.exitCode !== 0) {
            ...
            if (!applyStarted) {
                if (
                    driftCheckState === "running" ||
                    driftCheckState === "pending"
                ) {
                    options.setStepStatus("drift-check", "failed");
                }
                options.setStepStatus("apply", "skipped");
    ...
    const handleDeployEnvironment = async (
        ...
        const latestProjectStateConfig = loadProjectConfig(currentProject.path);
        const selectedRelease =
            latestProjectStateConfig?.releaseState?.environments?.[
                environmentId
            ] ??
            projectStateConfig?.releaseState?.environments?.[environmentId];
        const templateMeta = findProjectConfigForProject(currentProject);
        const deployConfig = templateMeta
            ...
            : null;
        const usesDeployTemplate = !!deployConfig?.environments.find(
        ...
        if (
            selectedRelease?.selectedReleaseTag &&
            !selectedRelease.selectedImageRef
        ) {
            setDeploymentError(
        ...
        const releaseTagResult = await ensureReleaseTagForProject(
            currentProject.path
        );
        ...
        const stepStatusById: Partial<
            Record<DeploymentStepId, DeploymentStepStatus>
        > = Object.fromEntries(
    ...
    const handleDeployLatestFromHome = async (
        environmentId: string
    ): Promise<void> => {
        setDeploymentError(null);

        const releaseConfigResult = loadProjectReleaseConfigWithError(
            currentProject.path
        );
        ...
        const tagsResult = await callShell(
            ...
            [
                "tag",
                "--list",
                `${releaseConfigResult.config.tagPrefix}*`,
                "--sort=-v:refname",
            ],
            {
                cwd: currentProject.path,
        ...
        const latestTag = (tagsResult.stdout ?? "")
            .split(/\r?\n/)
            .map((line) => line.trim())
            .find((line) => line.length > 0);
        if (!latestTag) {
            setDeploymentError(
                `No release tags found for prefix '${releaseConfigResult.config.tagPrefix}'. Create a release first.`
            );
            return;
        }
        ...
        if (!resolvedImage.ok) {
            setDeploymentError(
                `Release '${latestTag}' is not build-ready: ${resolvedImage.message}`
            );
            return;
        }

        setEnvironmentReleaseSelection(environmentId, {
            releaseTag: latestTag,
            imageRef: resolvedImage.imageRef,
            imageDigest: resolvedImage.imageDigest,
        });

        await handleDeployEnvironment(environmentId);
    };

    const handleEnvironmentActionFromHome = (environmentId: string): void => {
        const status = getEnvironmentStatus(environmentId).toLowerCase();
        if (
            status === "not deployed" ||
            status === "unknown" ||
            status === "down"
        ) {
            void handleDeployLatestFromHome(environmentId);
            return;
        }
        setConfirmDestroyEnvironmentId(environmentId);
    };

    const handleDestroyEnvironmentConfirm = async () => {
        if (!confirmDestroyEnvironmentId) return;
        const environmentId = confirmDestroyEnvironmentId;
        const args = [
            "deployments",
            "destroy",
            "--env",
        ...
        const result = await runDeploymentAction(
            args,
            `Destroyed environment '${environmentId}'.`
        );
        if (result.exitCode === 0) {
            const latestProjectStateConfig = loadProjectConfig(currentProject.path);
            if (latestProjectStateConfig?.environmentOutputs?.[environmentId]) {
                const nextEnvironmentOutputs = {
                    ...(latestProjectStateConfig.environmentOutputs ?? {}),
                };
                delete nextEnvironmentOutputs[environmentId];
                saveProjectConfig(currentProject.path, {
                    ...latestProjectStateConfig,
                    environmentOutputs: nextEnvironmentOutputs,
                });
                setDeploymentRefreshKey((v) => v + 1);
            }
        }
    ...
    const handleCreateReleaseTagSubmit = async (
        ...
        const backendProfile =
            config.integrations?.aws?.backend?.profile?.trim();
        ...
        if (!varsValidation.ok) {
            setReleaseBuildMonitor({
                tag: requestedTag,
                stage: "failed",
                message: `${varsValidation.message}${
                    bootstrapResult.configured
                        ? ""
        ...
        const result = await createReleaseTagForProject(
            currentProject.path,
            requestedTag
        );
        if (!result.ok) {
        ...
        const pushResult = await pushReleaseTagToOrigin(
            currentProject.path,
            result.tag
        );
        if (!pushResult.ok) {
        ...
        setReleaseBuildMonitor({
            ...
            stage: "completed",
            message:
                "Release build is ready. Release reference validated for deployment.",
            runUrl: resolvedImage.runUrl ?? monitorResult.runUrl,
            preflightSummary,
        });
        setDeploymentLogs((prev) => [
            ...prev.slice(-39),
            `Created release '${result.tag}'.`,
        ]);
        closeReleaseSelector();
    };
    ...
    const openReleaseSelector = async (environmentId: string) => {
        ...
        if (templateMeta) {
            ...
            if (deployConfigResult.exists && !deployConfigResult.config) {
                setDeploymentError(
                    `Template deploy config is invalid for '${
                        currentProject.type
                    }'. ${deployConfigResult.error ?? ""}`.trim()
                );
                setSelectedEnvironmentProvider(null);
                setAvailableDeployPresets([]);
                    ...
                    const presets = deployConfigResult.config.presets
                        .filter(
                            (preset) =>
                                ...
                                (!preset.provider ||
                                    preset.provider === deployEnv.provider)
                        )
                        .map((preset) => ({
                            id: preset.id,
                            label: preset.label,
        ...
        return (
            <PendingApplyView
                ...
                onCancel={() => {
                    setPendingApplyEnvironmentId(null);
                    setDeploymentNotice(
                        "Apply cancelled. Plan remains available for review."
                    );
                    setDeploymentSteps((prev) =>
                        prev.map((step) =>
                            step.status === "pending"
        ...
        return (
            <ReleaseSelectorView
                ...
                onCreateReleaseSubmit={(value) => {
                    const requestedTag =
                        value.trim() || suggestedReleaseTag.trim();
                    if (!requestedTag) {
                    ...
                    void (async () => {
                        ...
                        if (!targetEnvironmentId) {
                            setDeploymentError(
                                "No target environment selected for release."
                            );
                            return;
                        }
                        await handleCreateReleaseTagSubmit(
                            targetEnvironmentId,
                            requestedTag
                        );
                    })();
                ...
                onClear={() => {
                    setEnvironmentReleaseSelection(
                        editReleaseEnvironmentId,
                        {},
                        { replace: true }
                    );
    ...
    if (selectedEnvironmentId) {
        ...
        const envState =
            projectStateConfig?.deploymentState?.environments?.[
                selectedEnvironmentId
            ];
        const envOutputs =
            projectStateConfig?.environmentOutputs?.[selectedEnvironmentId] ??
            {};
        const lastSuccessfulApply = (
            projectStateConfig?.deploymentRunHistory ?? []
        )
            .filter(
                (run) =>
        ...
        const resolvedOutputs = Object.values(envOutputs)
            .filter(
                (record) =>
                    record.source === "providerOutput" ||
                    record.key === "APP_BASE_URL"
            )
            .slice(0, 8)
            .map((record) => {
                if (record.secretRef && record.secretRef.length > 0) {
                    return {
                        key: record.key,
                        value: `secret ref: ${record.secretRef}`,
                    };
                }
                if (
                ...
                if (typeof record.value === "undefined")
                    return { key: record.key, value: "(empty)" };
        ...
        return (
            <EnvironmentActionsView
                ...
                actionLocked={
                    deploymentInProgress ||
                    Date.now() < deploymentActionCooldownUntilRef.current
                }
                ...
                onDestroy={() =>
                    setConfirmDestroyEnvironmentId(selectedEnvironmentId)
                }
            />
        );
    }
    ...
    return (
        <DashboardHomeView
            ...
            answers={answers}
            localUrl={resolveOpenUrl(currentProject)}
            ...
            getEnvironmentStatusLabel={getEnvironmentStatusLabel}
            getEnvironmentUrl={getEnvironmentUrl}
            onSelectEnvironment={(environmentId) =>
                setSelectedEnvironmentId(environmentId)
            }
            onSelectEnvironmentAction={handleEnvironmentActionFromHome}
