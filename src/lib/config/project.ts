...
export type ProjectOutputType =
    ...
    | "json"
    | "secret_ref";
export type ProjectOutputSource =
    | "manualOverride"
    | "providerOutput"
    | "templateDefault";
...
function normalizeEnvironmentOutputs(
    raw: unknown
): ProjectEnvironmentOutputs | undefined {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) return undefined;
    const result: ProjectEnvironmentOutputs = {};
    for (const [environmentId, recordMap] of Object.entries(
        raw as Record<string, unknown>
    )) {
        if (
            !recordMap ||
            typeof recordMap !== "object" ||
            Array.isArray(recordMap)
        )
            continue;
...
function sourceHasPriority(
    next: ProjectOutputSource,
    current: ProjectOutputSource
): boolean {
    return OUTPUT_SOURCE_PRIORITY[next] >= OUTPUT_SOURCE_PRIORITY[current];
}

function normalizeDeploymentRunHistory(
    raw: unknown
): ProjectDeploymentRunRecord[] | undefined {
    ...
    for (const item of raw) {
        ...
        records.push({
            ...
            action: candidate.action as ProjectDeploymentAction,
            status: candidate.status as ProjectDeploymentRunStatus,
            actor:
            ...
            summary:
                candidate.summary && typeof candidate.summary === "object"
                    ? {
                          add:
                              typeof (candidate.summary as { add?: unknown })
                                  .add === "number"
                                  ? (candidate.summary as { add: number }).add
                                  : undefined,
                          change:
                              typeof (candidate.summary as { change?: unknown })
                                  .change === "number"
                                  ? (candidate.summary as { change: number })
                                        .change
                                  : undefined,
                          destroy:
                              typeof (
                                  candidate.summary as { destroy?: unknown }
                              ).destroy === "number"
                                  ? (candidate.summary as { destroy: number })
                                        .destroy
                                  : undefined,
                      }
                    : undefined,
            logs: Array.isArray(candidate.logs)
                ? candidate.logs.filter(
...
function normalizeDeploymentState(raw: unknown): DeploymentState | undefined {
    ...
    for (const [environmentId, value] of Object.entries(
        ...
        const activeLockCandidate =
            ...
            typeof (candidate.activeLock as { runId?: unknown }).runId ===
                "string" &&
            typeof (candidate.activeLock as { acquiredAt?: unknown })
                .acquiredAt === "string"
                ? {
                      runId: (candidate.activeLock as { runId: string }).runId,
                      acquiredAt: (
                          candidate.activeLock as { acquiredAt: string }
                      ).acquiredAt,
                  }
                : undefined;
        environments[environmentId] = {
            lastPlanAt:
                typeof candidate.lastPlanAt === "string"
                    ? candidate.lastPlanAt
                    : undefined,
            ...
            lastForceUnlockAt:
                typeof candidate.lastForceUnlockAt === "string"
                    ...
                    : undefined,
            lastReportedAt:
                typeof candidate.lastReportedAt === "string"
                    ? candidate.lastReportedAt
                    : undefined,
            lastStatusUpdatedAt:
...
function normalizeReleaseState(raw: unknown): ProjectReleaseState | undefined {
    ...
    for (const [environmentId, value] of Object.entries(
        ...
        environments[environmentId] = {
            selectedImageRef:
                typeof candidate.selectedImageRef === "string"
                    ? candidate.selectedImageRef
                    : undefined,
            selectedImageDigest:
                typeof candidate.selectedImageDigest === "string"
                    ? candidate.selectedImageDigest
                    : undefined,
            selectedReleaseTag:
                typeof candidate.selectedReleaseTag === "string"
                    ? candidate.selectedReleaseTag
                    : undefined,
...
export function loadProjectConfig(path: string): TzProjectConfig | null {
    ...
    const environmentOutputs = normalizeEnvironmentOutputs(
        config.environmentOutputs
    );
    const deploymentRunHistory = normalizeDeploymentRunHistory(
        config.deploymentRunHistory
    );
...
export function upsertProjectEnvironmentOutputs(
    ...
    for (const write of writes) {
        ...
        const isGeneratedCredential =
            write.isGeneratedCredential ??
            current?.isGeneratedCredential ??
            false;
